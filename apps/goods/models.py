...
class GoodsType(BaseModel):
    """商品类型模型类"""
    name = models.CharField(max_length=20, verbose_name='商品类型名称')
    logo = models.CharField(max_length=20, verbose_name='类型标识')
    image_url = models.URLField(default='', verbose_name='商品类型图片')
...
class IndexGoodsType(BaseModel):
    """首页分类商品展示模型类"""
    ...
    sku = models.ForeignKey('GoodsSKU', on_delete=models.CASCADE, verbose_name='商品')
    display_type = models.SmallIntegerField(default=1, choices=DISPLAY_TYPE_CHOICES, verbose_name='商品展示方式')
    index = models.SmallIntegerField(default=0, verbose_name='展示顺序')
